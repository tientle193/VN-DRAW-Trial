...
class DroughtState {
	...
	years = $derived.by(() => {
		...
		return this.currentData ? availableYears(this.currentData, this.selectedIndicator) : ALL_YEARS;
	});

	continuousDomain: Domain | null = $derived(
		this.category === 'drought' && this.selectedIndicator !== 'dri' && this.currentData
			? computeContinuousDomain(this.currentData, this.selectedIndicator, this.selectedYear)
			: null
	);
	...
	async ensureLoaded(indicator: IndicatorId, year: number) {
		...
		try {
			...
			if (indicator !== 'dhi') {
				// Single multi-year files: populate cache for all years
				for (const y of ALL_YEARS) {
					this.datasets.set(cacheKey(indicator, y), fc);
				}
			} else {
